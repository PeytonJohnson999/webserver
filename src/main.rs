        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                //Basic CSS
                    ...
                    send_page("css/style.css", ContentType::css, &mut stream)
                        .await
                        .unwrap();
                } else if method == Method::GET {
                    ...
                    match send_page(
                        path.clone(),
                        ContentType::from_str(&path).unwrap(),
                        &mut stream,
                    )
                    .await
                    {
                        ...
                        Err(e) => match e.root_cause().to_string().as_str() {
                            "No such file or directory (os error 2)" => {
                                if path.contains("html") {
                                    send_page("html/404.html", ContentType::html, &mut stream)
                                        .await
                                        .unwrap()
                                }
                            }
